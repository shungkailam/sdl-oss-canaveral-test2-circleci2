...
cd /tmp/go/src/cloudservices && go tool cover -html=/tmp/eventserver.cov -o /tmp/eventserver_cov.html && cd -

echo "Upload coverage html file to S3."
cd /tmp/go/src/cloudservices && go run cloudmgmt/code_coverage/uploadToS3.go -filePath /tmp/eventserver_cov.html && cd -
