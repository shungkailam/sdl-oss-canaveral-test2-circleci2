...
/usr/src/app/operatorserver_cov $@
CODE_COV_PID=$!
...
cd /tmp/go/src/cloudservices && go tool cover -html=/tmp/operator.cov -o /tmp/operator_cov.html && cd -

echo "Upload coverage html file to S3."
cd /tmp/go/src/cloudservices && go run cloudmgmt/code_coverage/uploadToS3.go -filePath /tmp/operator_cov.html && cd -
